...
TypeScript/Express API that orchestrates slot-to-slot deployments, persists state via Prisma, streams events over WebSockets, and regenerates the Caddy router on demand.
...
- `POST /api/services` expects both `slot-a` and `slot-b` entries inside `environments`.
...
- `ROUTER_DOMAIN` controls the public hostnames that Caddy serves (default `switchyard.localhost`, producing `sample-api.switchyard.localhost`, `slot-a.sample-api.switchyard.localhost`, and `slot-b.sample-api.switchyard.localhost`). Point it at your internal DNS zone if you own one.
...
- `PORT_RANGE_START`/`PORT_RANGE_END` define the inclusive host-port range Switchyard scans when assigning slot host ports.

## Service Scope
- Switchyard currently manages a single service pair (`slot-a` + `slot-b`). Calling `POST /api/services` creates the initial pair if none exists or overwrites the existing pair; additional pairs are blocked for now to keep the workflow simple.

## Routing Automation (Caddy)
- `src/lib/caddyfile.ts` emits router configs for both slot subdomains:
  - `slot-a.<service>.switchyard.localhost` -> Slot A
  - `slot-b.<service>.switchyard.localhost` -> Slot B
  - `<service>.switchyard.localhost` -> active slot
...
1. Deploy Docker images to the non-active slot (`/services/:id/deployments`). The backend currently logs `STARTING DOCKER IMAGE: ...` as a placeholder for the future Docker SDK.
2. Validate that slot endpoint.
3. Promote it by switching traffic with `/services/:id/switch` (or the UI). Rolling back simply switches traffic back to the other slot.
...
- Service-level metadata (description, repository URL, health endpoint) can only be edited when **both** slot containers are stopped. This keeps the health monitor, router, and operators aligned on which build is live.
